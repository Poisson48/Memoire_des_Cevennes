...
function renderCreate(qi) {
  ...
  const refs = qi.refs || {};
  const sub = item.submittedBy ? `par <strong>${escapeHtml(item.submittedBy.name || item.submittedBy.pseudo || item.submittedBy.email || '?')}</strong> · ` : '';
...
function renderEdit(qi) {
  ...
  const sub = edit.submittedBy ? `par <strong>${escapeHtml(edit.submittedBy.name || edit.submittedBy.pseudo || edit.submittedBy.email || '?')}</strong> · ` : '';
